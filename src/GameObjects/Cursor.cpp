...
namespace pyramidnight {
	...
	Cursor::Cursor(sf::Font& font) : mFont(font) {
		...
		cursorSize.x =	static_cast<float>(mFont.getGlyph('?', CUR_FONT_SZ, false).bounds.size.x);
		cursorSize.y =	static_cast<float>(mFont.getGlyph('?', CUR_FONT_SZ, false).bounds.size.y);
